...
namespace DerivSmartRobot.Robots
{
    ...
    public class RsiRobot : BaseRobot, IRobotOperations, IRsiRobot
    {
        ...
        public RsiRobot(ITradeService tradeService) :base(tradeService)
        {
        ...
        public void VerifyAndBuy(ResponseMessage message)
        {
            ...
            if (ActionStockRsi == "buy" && candles.TakeLast(2).First().IsBullish && candles.TakeLast(2).First().BodyPct > 0.2 && ActionParabolicSar == "buy" )
            {
                if (DateTime.Now <= BuyDeadLine)
                {
                    ...
                    this.MakeAProposal(ContractType.CALL, _tradeService.RobotConfigutarion.CalledFrom != null ?  5 : 1, _tradeService.RobotConfigutarion.CalledFrom != null ? "t" :"m");
                    ActionStockRsi = "";
                    ActionParabolicSar = "";
                    BuyDeadLine = null;
                }
            ...
            if (ActionStockRsi == "sell"  && candles.TakeLast(2).First().IsBearish && candles.TakeLast(2).First().BodyPct > 0.2 && ActionParabolicSar == "sell")
            {
                if (DateTime.Now <= BuyDeadLine)
                {
                    ...
                    this.MakeAProposal(ContractType.PUT, _tradeService.RobotConfigutarion.CalledFrom != null ? 5 : 1, _tradeService.RobotConfigutarion.CalledFrom != null ? "t" : "m");
                    ActionStockRsi = "";
                    ActionParabolicSar = "";
                    BuyDeadLine = null;
                }
